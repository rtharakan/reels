              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {card.sharedFilms.map((film: { id: string; title: string; posterUrl?: string | null }) => (
                  <div key={film.id} className="flex-shrink-0">
                    {film.posterUrl ? (
                      <div className="aspect-[2/3] w-[72px] overflow-hidden rounded-xl">
                        <Image src={film.posterUrl} alt={`${film.title} poster`} width={72} height={108} className="h-full w-full object-cover" />
                      </div>
                    ) : (
                      <div className="flex h-[108px] w-[72px] items-center justify-center rounded-xl bg-[var(--bg-accent)] text-[10px] text-[var(--text-muted)] p-1 text-center">
                        {film.title}
                      </div>
                    )}
                  </div>
